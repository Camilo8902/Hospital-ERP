...
export default function ConsultationPage({ params }: { params: { id: string } }) {
  ...
  const [existingRecord, setExistingRecord] = useState<ClinicalRecord | null>(null);
  ...
  // Cargar datos de la cita y paciente
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Cargar paciente
        const patResponse = await fetch(`/api/patients/${aptData.appointment.patient_id}`);
        if (patResponse.ok) {
        ...
        const recordResponse = await fetch(`/api/clinical-records/appointment/${appointmentId}`);
        if (recordResponse.ok) {
          const recordData = await recordResponse.json();
          if (recordData.record) {
            const record = recordData.record as ClinicalRecord;
  ...
  const handleSave = async (complete: boolean) => {
    if (complete && !confirm('¿Deseas completar la consulta? Esto cambiará el estado de la cita a "Completada".')) {
      return;
    }
    ...
    try {
      const response = await fetch('/api/clinical-records', {
        ...
        body: JSON.stringify({
          id: recordId, // Incluir ID si existe registro previo
          ...formData,
      ...
      // Guardar el ID del registro para futuras actualizaciones
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Consulta Completada</h2>
        <p className="text-gray-500 mt-2">La historia clínica ha sido guardada exitosamente.</p>
        <Link href="/dashboard/appointments" className="btn-primary mt-6">
          Volver a Citas
        </Link>
      </div>
    );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {existingRecord ? 'Continuar Consulta' : 'Consulta Médica'}
            </h1>
        ...
        <div className="flex items-center gap-3">
          {recordId && (
            <span className="text-sm text-gray-500">
              Borrador guardado {lastSaved ? `a las ${lastSaved}` : ''}
            </span>
          )}
          <span className={`badge ${
            ...
            {appointment.status === 'in_progress' ? 'En Progreso' : appointment.status}
          </span>
        </div>
      </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{patient.dob ? new Date().getFullYear() - new Date(patient.dob).getFullYear() : '-'} años</p>
                </div>
                <div>
            ...
            <div className="card-body space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={5}
                  placeholder="Describa detalladamente la evolución del síntomas, características, factores agravantes o alivio, etc."
                  className="input"
                />
              </div>
            </div>
          ...
          {expandedSections.physicalExam && (
            <div className="card-body">
              <textarea
                ...
                rows={6}
                placeholder="Describa los hallazgos de la exploración física: estado general, signos vitales (ya registrados), inspección, palpación, percusión, auscultación de cada sistema explorado..."
                className="input"
              />
            </div>
          )}
            ...
            <div className="card-body space-y-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Buscar por código o descripción (ej: J06.9, gripe, diabetes)"
                  className="input"
                />
                ...
                {showIcdDropdown && icdSearch.length >= 2 && icdCodes.length === 0 && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-center text-gray-500">
                    No se encontraron códigos ICD-10
                  </div>
                )}
              </div>
            ...
            <div className="card-body space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Describa el plan de tratamiento: medicamentos, procedimientos, cambios en estilo de vida, etc."
                  className="input"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Instrucciones para el paciente: dieta, actividad, signos de alarma, etc."
                  className="input"
                />
              </div>
              ...
              {/* Links para crear recetas y órdenes de laboratorio */}
              {appointment && (
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <Link
                    href={`/dashboard/pharmacy/prescriptions/new?appointment_id=${appointmentId}&patient_id=${patient?.id}`}
                    className="flex items-center gap-2 p-4 bg-primary-50 rounded-lg text-primary-700 hover:bg-primary-100 transition-colors"
                  >
                    <Pill className="w-5 h-5" />
                    <div>
                      <span className="font-medium block">Crear Receta</span>
                      <span className="text-xs text-primary-600">Emitir prescripción médica</span>
                    </div>
                  </Link>
                  <Link
                    href={`/dashboard/lab/new?appointment_id=${appointmentId}&patient_id=${patient?.id}`}
                    className="flex items-center gap-2 p-4 bg-teal-50 rounded-lg text-teal-700 hover:bg-teal-100 transition-colors"
                  >
                    <FlaskConical className="w-5 h-5" />
                    <div>
                      <span className="font-medium block">Orden de Laboratorio</span>
                      <span className="text-xs text-teal-600">Solicitar pruebas de laboratorio</span>
                    </div>
                  </Link>
                </div>
              )}
          ...
          {expandedSections.notes && (
            <div className="card-body">
              <p className="text-sm text-gray-500 mb-2">
                Estas notas solo son visibles para doctores y personal autorizado
              </p>
              <textarea
                value={formData.private_notes}
                onChange={(e) => setFormData(prev => ({ ...prev, private_notes: e.target.value }))}
                rows={3}
                placeholder="Notas internas, observaciones personales, recordatorios..."
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {saving ? 'Completando...' : 'Completar Consulta'}
            </button>
          </div>
        </div>
      </form>
